...
var ReactFiberBeginWork = function (config$$1, hostContext, hydrationContext, scheduleWork, computeExpirationForFiber) {
  var shouldSetTextContent = config$$1.shouldSetTextContent,
      useSyncScheduling = config$$1.useSyncScheduling,
      shouldDeprioritizeSubtree = config$$1.shouldDeprioritizeSubtree;
...
var ReactFiberCompleteWork = function (config$$1, hostContext, hydrationContext) {
  var createInstance = config$$1.createInstance,
      createTextInstance = config$$1.createTextInstance,
      appendInitialChild = config$$1.appendInitialChild,
      finalizeInitialChildren = config$$1.finalizeInitialChildren,
      prepareUpdate = config$$1.prepareUpdate,
      mutation = config$$1.mutation,
      persistence = config$$1.persistence;
...
var ReactFiberCommitWork = function (config$$1, captureError) {
  var getPublicInstance = config$$1.getPublicInstance,
      mutation = config$$1.mutation,
      persistence = config$$1.persistence;
...
var ReactFiberHostContext = function (config$$1) {
  var getChildHostContext = config$$1.getChildHostContext,
      getRootHostContext = config$$1.getRootHostContext;
...
var ReactFiberHydrationContext = function (config$$1) {
  var shouldSetTextContent = config$$1.shouldSetTextContent,
      hydration = config$$1.hydration;
...
var ReactFiberScheduler = function (config$$1) {
  var hostContext = ReactFiberHostContext(config$$1);
  var hydrationContext = ReactFiberHydrationContext(config$$1);
  var popHostContainer = hostContext.popHostContainer,
      popHostContext = hostContext.popHostContext,
      resetHostContainer = hostContext.resetHostContainer;

  var _ReactFiberBeginWork = ReactFiberBeginWork(config$$1, hostContext, hydrationContext, scheduleWork, computeExpirationForFiber),
      beginWork = _ReactFiberBeginWork.beginWork,
      beginFailedWork = _ReactFiberBeginWork.beginFailedWork;

  var _ReactFiberCompleteWo = ReactFiberCompleteWork(config$$1, hostContext, hydrationContext),
      completeWork = _ReactFiberCompleteWo.completeWork;

  var _ReactFiberCommitWork = ReactFiberCommitWork(config$$1, captureError),
  ...
  var now = config$$1.now,
      scheduleDeferredCallback = config$$1.scheduleDeferredCallback,
      cancelDeferredCallback = config$$1.cancelDeferredCallback,
      useSyncScheduling = config$$1.useSyncScheduling,
      prepareForCommit = config$$1.prepareForCommit,
      resetAfterCommit = config$$1.resetAfterCommit;
...
var ReactFiberReconciler$1 = function (config$$1) {
  var getPublicInstance = config$$1.getPublicInstance;

  var _ReactFiberScheduler = ReactFiberScheduler(config$$1),
...
var models = lib.scan(applyUpdate, initialModel, update);

var view = createView(createActions(update));
var element = document.getElementById("app");
models.map(function (model) {
  return reactDom.render(view(model), element);
});
